...
<script type="text/javascript">
    ...
    $(function (){
        ...
        if(kf_name != "" && kf_enname != ""){
            ...
            if(window.location.search != "" && getCookie("kf_name") != "undefined" ){
                window.location.href = "gradenew.aspx" + window.location.search + "&name=" + getCookie("kf_name") + "&enname=" + getCookie("kf_enname")
            // return false
            }
        }
    ...
    var showResponse = function (responseText, statusText, xhr, $form) {
        //console.log("responseText:" + responseText + "," + JSON.parse(responseText).Model + "," + JSON.parse(responseText).error.length)
        if (JSON.parse(responseText).Model == "" && JSON.parse(responseText).error.length >= 1 ) {
            alert("温馨提示："+JSON.parse(responseText).error)
        }
